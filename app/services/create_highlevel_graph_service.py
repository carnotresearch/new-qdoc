...
def parse_important_sentences_file(file_path: str) -> List[Document]:
    ...
    # Return the entire content as a single document
    if content:
...
def process_text_file(input_file: str, session_id: str, file_name: str, meta: Optional[Dict] = None) -> List[GraphDocument]:
    """
    Main function to process a text file containing important sentences and store results in Neo4j with complete session isolation
    
    Args:
        input_file: Path to the input text file (imp_sents.txt)
    ...
    if os.path.exists(cache_file):
        ...
        # Use the updated function to parse the important sentences file
        documents = parse_important_sentences_file(input_file)
